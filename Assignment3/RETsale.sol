...
// Importing the token contract file
import "./RET.sol";

// Defining the contract
contract RETSale {
    address admin; // The address of the admin
    RoyalEntryToken public tokenContract; // The contract of the token
    ...
    event Entry(address _enterer, uint256 _amount); // Event that is emitted when someone enters the coffee shop

    // Constructor of the contract that initializes with the address of the token contract and sets the token price
    constructor(RoyalEntryToken _tokenContract, uint256 _tokenPrice) {
        admin = msg.sender; // The admin is the creator of the contract
        tokenContract = _tokenContract; // Set the token contract
    ...
    // Function that sells the tokens to the buyer
    ...
    // Function that allows a user to sell tokens for ETH
